...
FROM (
SELECT pharmacist_sin AS sin, 'Pharmacist' AS EmployeeRole FROM Pharmacist WHERE pharmacist_sin IN (SELECT employee_sin FROM Schedule WHERE end_date IS NULL)
UNION ALL
...
if ($result->num_rows > 0) {
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    ...
    echo "<td>" . $row['Employees_Never_Infected'] . "</td>";
    echo "</tr>";
}
    echo "</table>";
} else {
